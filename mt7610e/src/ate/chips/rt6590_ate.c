...
/* ralink debug */
	...
	rtmp_mac_set_ctrlch(pAd, ext_ch); /* ralink debug */
	ate_bbp_set_ctrlch(pAd, ext_ch); /* ralink debug */
}
    ...
    IN PRTMP_ADAPTER pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		(" [%s] Channel = %d, pATEInfo->TxWI.TxWIBW = %d , pATEInfo->RFFreqOffset = %d, "
		"pATEInfo->TxPower0 = %d, pATEInfo->TxPower1 = %d\n", 
		__FUNCTION__, Channel, pATEInfo->TxWI.TxWIBW, pATEInfo->RFFreqOffset,
	...
	IN PRTMP_ADAPTER pAd) 
{
	PATE_INFO pATEInfo = &(pAd->ate);
	UINT32 parameter = 0;
	INT32 BandWidthSel = pATEInfo->TxWI.TxWIBW;
	...
	if ((pATEInfo->Channel > 14) || (BandWidthSel == BW_80))
	{
		...
		DBGPRINT(RT_DEBUG_OFF, ("channel = %d, bandwidth = %d\n"
				, pATEInfo->Channel, BandWidthSel));
		DBGPRINT(RT_DEBUG_OFF, ("only support DPD calibration for 2.4G band\n"));
		return;	
	}
	...
	IN BOOLEAN bFullCal)
{
	...
	if (IS_MT7610(pAd))
	{
		if (Channel <= 14)
		{
			DBGPRINT(RT_DEBUG_OFF, ("error - 7610: Channel = %d\n", Channel));
			return;
		}
	}
	...
	if (IS_MT7630(pAd))
	{
		if (Channel > 14)
		{
			DBGPRINT(RT_DEBUG_OFF, ("error - 7630: Channel = %d\n", Channel));
			return;
		}
	}
