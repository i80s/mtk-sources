	...
	IN  UINT32                DevType)
{
	...
	if (dev_p)
	{
		...
		if (isGoOn)
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Group remove stop mcc \n"));
//				pAd->chipCap.tssi_enable = TRUE; //let host do tssi	
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("GO remove & switch to Infra BW = %d  pAd->StaCfg.wdev.CentralChannel %d \n",pAd->StaCfg.wdev.bw,pAd->StaCfg.wdev.CentralChannel));
...
/*after p2p cli connect , neet to change to default configure*/
		...
		if (pAd->flg_apcli_init)
		{
			...
			if (pAd->Mlme.bStartMcc == TRUE)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("GC remove stop mcc \n"));
				pAd->chipCap.tssi_enable = TRUE; //let host do tssi	
				Stop_MCC(pAd,0);
				pAd->Mlme.bStartMcc = FALSE;
			}else
			//if (pAd->Mlme.bStartScc == TRUE)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("GC remove & switch to Infra BW = %d  pAd->StaCfg.wdev.CentralChannel %d \n",pAd->StaCfg.wdev.bw,pAd->StaCfg.wdev.CentralChannel));
				pAd->Mlme.bStartScc = FALSE;
				AsicSwitchChannel(pAd, pAd->StaCfg.wdev.CentralChannel, FALSE);
				AsicLockChannel(pAd, pAd->StaCfg.wdev.CentralChannel);					
				bbp_set_bw(pAd, pAd->StaCfg.wdev.bw);
