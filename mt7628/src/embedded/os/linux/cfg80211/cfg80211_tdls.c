	...
	UINT						Data)
{
	...
	if(Data == tdls_insert_entry)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s() - EXCEED MAX_NUM_OF_CFG_TDLS_ENTRY %d. Abort this setup\n", __FUNCTION__,MAX_NUM_OF_CFG_TDLS_ENTRY));
			return FALSE;
		}
		...
		if (pMacEntry)
		{
			...
			//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("=====>  %s() line: %d  supRateLen: %d ExtRateLen:%d Htlen:%d\n"	, __FUNCTION__,__LINE__,pAd->StaActive.SupRateLen,pAd->StaActive.ExtRateLen,HtLen));  //Kyle Debug
			...
			{
				//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("=====>  %s() line: %d  TDLS setting Encryption\n", __FUNCTION__,__LINE__));  //Kyle Debug
				...
				if ( pMacEntry->Aid > 1)
				{
					...
					RTMPSetWcidSecurityInfo(pAd,
        				...
        				if (pAd->chipCap.hif_type == HIF_MT)
				            CmdProcAddRemoveKey(pAd, 0, BSS0, CIPHER_AES, pMacEntry->wcid, PAIRWISEKEYTABLE, &pMacEntry->PairwiseKey, pMacEntry->Addr);
					#endif
					//NdisMoveMemory(&pMacEntry->PairwiseKey, &PairwiseKey, sizeof(CIPHER_KEY));
...
BOOLEAN cfg_tdls_UAPSDP_AsicCanSleep(PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("<======  %s() out\n", __FUNCTION__));
...
void cfg_tdls_rcv_PeerTrafficIndication(PRTMP_ADAPTER pAd,u8 dialog_token,u8 *peer)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s(): can't find valid link ID, tdls link to (%02X:%02X:%02X:%02X:%02X:%02X) doesn't exist.\n",__FUNCTION__,PRINT_MAC(peer)));
		return;
	}
...
void cfg_tdls_send_PeerTrafficIndication(PRTMP_ADAPTER pAd, u8 *peer)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls uapsd> traffic ind of index %d was sent before!\n",tdls_entry_link_index));
		return; /* has sent it */
	}
...
int cfg_tdls_build_frame(PRTMP_ADAPTER pAd,u8 *peer,u8 dialog_token,u8 action_code,u16 status_code,const u8 *extra_ies,size_t extra_ies_len,BOOLEAN send_by_tdls_link,u8 tdls_entry_link_index,u8 reason_code)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("======>  %s() \n", __FUNCTION__));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s() - TDLSEntry[%d] Initiator = %d\n", __FUNCTION__,tdls_entry_link_index,pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.TDLSEntry[tdls_entry_link_index].bInitiator));
	/* Allocate buffer for transmitting message */
	NStatus = MlmeAllocateMemory(pAd, &pOutBuffer);
	if (NStatus != NDIS_STATUS_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ACT - TDLS_SetupRequestAction() allocate memory failed \n"));
		return NStatus;
	}
	...
	if(send_by_tdls_link == TRUE) 								//only uapsd PTR is sent by this way
	{
#ifdef UAPSD_SUPPORT
	...
	IN PUCHAR arg)
{
	...
	for (i=0; i<MAX_NUM_OF_CFG_TDLS_ENTRY; i++)
	{
		if ((pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.TDLSEntry[i].EntryValid == TRUE))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TDLS Entry %d is valid, bInitiator = %d \n"
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-4d", (int)pEntry->apidx));
...
VOID cfg_tdls_rx_parsing(PRTMP_ADAPTER pAd,RX_BLK *pRxBlk)
{
		...
		&& NdisEqualMemory(TDLS_LLC_SNAP_WITH_CATEGORY, pRxBlk->pData, LENGTH_802_1_H + 2))
	{
		...
		if(TDLSType == TDLS_ACTION_CODE_SETUP_REQUEST|| TDLSType == TDLS_ACTION_CODE_SETUP_RESPONSE)
		{
			...
			if(i==MAX_NUM_OF_CFG_TDLS_ENTRY)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s() - TDLS LINK NUMBER EXCEED, WE ONLY SUPPORT CONCURRENT %d LINKs, Discard this setup\n", __FUNCTION__,MAX_NUM_OF_CFG_TDLS_ENTRY));
			...
			else
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s() - TDLSEntry[%d] Initiator = %d\n", __FUNCTION__,tdls_entry_index,pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.TDLSEntry[tdls_entry_index].bInitiator));
			...
			{	
				...
				{	
					...
					case IE_VENDOR_SPECIFIC:
						...
						,__FUNCTION__, pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.TDLSEntry[tdls_entry_index].QosCapability, PRINT_MAC(peerMAC)));
						break;
					case IE_HT_CAP:
...
#else
	cfg_tdls_build_frame(pAd,peerAddr,1,TDLS_ACTION_CODE_TEARDOWN,0,NULL,0,FALSE,0,25);
	mdelay(1);
