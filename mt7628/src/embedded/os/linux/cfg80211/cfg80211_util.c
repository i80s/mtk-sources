	...
	IN VOID *pRatesOrg)
{
	...
	if (pChannels == NULL && NumOfChan)
	{
		...
		if (!pChannels)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("80211> ieee80211_channel allocation fail!\n"));
			return FALSE;
		}
	}
	...
	if (pRates == NULL && NumOfRate)
	{
		...
		if (!pRates)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("80211> ieee80211_rate allocation fail!\n"));
			return FALSE;
		}
	}
