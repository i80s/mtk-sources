...
)
{
	...
	UINT16 tmp16;
	UCHAR *pos;
	UCHAR L2PAD,PHYMODE,BW,ShortGI,MCS,LDPC,LDPC_EX_SYM,AMPDU,STBC,RSSI1,*pDevName, Channel, CentralChannel, sideband_index;
	CHAR MaxRssi;
	UINT32 timestamp;

	NdisZeroMemory(pAd->data_radiotap_header,sizeof(pAd->data_radiotap_header));
	...
	mtk_rt_hdr = (struct mtk_radiotap_header *)pAd->data_radiotap_header;
	pAd->data_radiotap_header_len = sizeof(*mtk_rt_hdr) + varlen;
	NdisZeroMemory(mtk_rt_hdr, sizeof(*mtk_rt_hdr) + varlen);
		...
		*pos |= (LDPC_EX_SYM?IEEE80211_RADIOTAP_VHT_FLAG_LDPC_EXTRA_OFDM_SYM:0);
		...
		if (BW == 0) {
			*pos = 0;
			...
			DBGPRINT(RT_DEBUG_ERROR, ("%s:unknown bw(%d)\n", __FUNCTION__, BW));
		}
...
#endif /* DOT11_VHT_AC */
	hex_dump("++++++ radiotap_header ",pAd->data_radiotap_header,pAd->data_radiotap_header_len);
}
#endif 